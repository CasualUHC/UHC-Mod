...
object MathUtils {
    ...
    private class DispersedBlockPosIterator(
        ...
        override fun hasNext(): Boolean {
            return this.branch < this.branches
        }

        override fun next(): MutableBlockPos {
            if (this.step == 1) {
                this.current.set(this.center)
            }
            val angle = 2 * Math.PI * this.branch / this.branches
            val ds = (this.step * this.distance * sin(angle)).toInt()
            val dc = (this.step * this.distance * cos(angle)).toInt()
            when (this.normal) {
